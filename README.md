...
`updatedAt`. Если облако перестало принимать вход (refresh-токен отозван или
протух), на «Сегодня» появляется плашка «Облако отключено — войти» и живёт до
нового входа, переживая перезапуск: отсутствие сети — это очередь, а не плашка.
`anon`-ключ в `sync.js` публичный по назначению — доступ к данным закрывают
политики RLS выше.

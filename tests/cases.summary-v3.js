...
(function () {
  ...
  describe('2.7.0 шаблон: ИТОГ печатается в форме v3', function () {
    ...
    ok(p.indexOf('Слова (3–5): ') > 0, 'слов 3–5 (2.7.8), а не 6–8');
  ...
  describe('2.7.3 итог: у бизнеса появились свои категории', function () {
    fresh();
    eq(State.trackHasCats('biz'), true, 'у biz теперь пять своих категорий');
    eq(State.trackHasCats('cs'), false, 'а у информатики своих нет');
    eq(State.trackHasCats('write'), true, 'у письма есть');

    var res = State.applySummary('B5.1', summary({
      debts: ['Б3 — путает markup и margin', 'П6 — ярлык вместо предложения']
    }), { date: '2026-08-20' });
    eq(res.created, 1, 'свой код принят, чужой отброшен');
    eq(byDid('D-1').cat, 'Б3', 'категория бизнеса');
    eq(byDid('D-1').track, 'biz', 'и дорожка своя');
    eq(res.dropped[0].why, 'код чужой дорожки', 'код письма на бизнесе не проходит');

    // латинская B приводится к кириллице, как M и O
    State.applySummary('B5.2', summary({ debts: ['B5 — питч без ask'] }), { date: '2026-08-21' });
    eq(byDid('D-2').cat, 'Б5', 'латинский код прочитан');
  });

...
(function () {
  ...
  describe('2.6.4: дедлайн далеко — старое поведение сохраняется', function () {
    ...
    // 2.7.8 (Б4): сцена — режим «Лето» (как у живой копии 27.08), и суббота
    // 29.08 — учебный день шаблона: у Б1 два урока на три дня (чт, пт, сб) —
    // правило молчит до пятницы; в режиме «Школа» — счёт 2.7.7
    eq(State.mode(), 'summer', 'сцена — режим «Лето»');
    eq(Waterfall.ruleDeadline(T), null, '2.7.8: летом у Б1 два урока на три учебных дня — молчит');
    var fri = Waterfall.ruleDeadline('2026-08-28');
    eq(fri && fri.reason.text, 'дедлайн: Б1 через 1 день, осталось 2 урока',
      '2.7.8: в пятницу два урока на два дня (пт, сб) — горит');
    State.setMode('school');
    var thu = Waterfall.ruleDeadline(T);
    eq(thu && thu.reason.text, 'дедлайн: Б1 через 2 дня, осталось 2 урока',
      '2.7.7: в режиме «Школа» у Б1 два урока на два будня до срока — как было');
  });

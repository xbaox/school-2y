...
(function () {
  ...
  describe('сжатие Ф0: водопад пропущенные не назначает', function () {
    ...
    // B3.4 пропущен → дорожка write в Ф0 закончилась
    eq(State.nextLessonInTrack('write'), null, 'B3.4 пропущен — на дорожке write уроков Ф0 нет');
  ...
  describe('сжатие Ф0: новые дедлайны и миграция', function () {
    ...
    eq(old.meta.version, 2, 'версия схемы поднята');
  ...
  describe('каркас Ф1: блоки переименованы', function () {
    fresh();
    eq(State.block('B7').title, 'Многочлены-1: деление и теорема об остатке', 'Б7');
    eq(State.block('B8').title, 'Чтение: главная мысль + язык фидбека учителя', 'Б8');
    eq(State.block('B9').title, 'Преобразования графиков и обратные функции', 'Б9');
    eq(State.block('B10').title, 'Абзац-мнение и заметка OSSLT', 'Б10');
    eq(State.block('B11').title, 'Рациональные функции и асимптоты', 'Б11');
    eq(State.block('B12').title, 'Маркетинг: 4P — поддержка BMI3C', 'Б12');
    eq(State.block('B13').title, 'Тригонометрия: радианы и графики sin/cos', 'Б13');
    eq(State.block('B14').title, 'OSSLT-генеральная — до ноябрьского окна', 'Б14');
    eq(State.block('B15').title, 'Тождества, теоремы синусов и косинусов', 'Б15');
    eq(State.block('B16').title, 'Финалы семестра: экзамен MHF4U', 'Б16');
    eq(State.blockLessons('B7').length, 0, 'уроки Ф1 придут отдельным пакетом');
  });

  describe('конкурсный урок: движок понимает type: \'contest\'', function () {
    fresh();
    var l = CONTENT.lesson('B2.1');
    var keepType = l.type, keepWriting = l.writing;
    l.type = 'contest';
    ...
    // формат итога тот же
    ok(p.indexOf('=== ИТОГ УРОКА B2.1 ===') > 0, 'итог по обычному формату');
    ok(p.indexOf('Погашено: ') > 0, 'со всеми полями');
    ok(p.indexOf('[КОНТРАКТ ПРЕПОДАВАТЕЛЯ — 16 правил') > 0, 'контракт на месте');

    l.type = keepType;
    l.writing = keepWriting;

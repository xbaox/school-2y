...
(function () {
  ...
  describe('конкурсный урок: движок понимает type: \'contest\'', function () {
    ...
    ok(p.indexOf('Ступень: ') === p.indexOf('Ступень: '), 'ступень в контексте есть');
    ok(p.indexOf('Формат: конкурсный урок') > 0, 'строка контекста объясняет формат');
    ok(p.indexOf('[ЗАДАЧИ]') > 0, 'задачи блока К напечатаны');
    ok(p.indexOf('Часть A') > 0 && p.indexOf('Часть B') > 0, 'обе части');

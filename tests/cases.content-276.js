...
(function () {
  ...
  describe('2.7.6 Э7: подписи блоков', function () {
    ...
    // 2.8.1: подписи — по разделу «Что ещё меняется контентом» пакета 2.8.1
    eq([State.block('B11').note, State.block('B13').note],
      ['Опорные задания и глоссарий — есть (2.8.1).', 'Опорные задания и глоссарий — есть (2.8.1).'], 'Б11 и Б13');
    eq(State.block('B12').note, 'Пьеса и роман в ENG2D идут до 18.12, экзамен в конце января. ' +
      'Опорные задания — из окна английского, пакет 2.8.2.', 'Б12');
    eq(State.block('B14').note, 'OSSLT — конец ноября 2026 (outline ENG2D); точная дата до 15.10. Если раньше 25.11 — ' +
      'дедлайн блока 15.11. Опорные задания — из окна английского, пакет 2.8.2.', 'Б14');
    ok(/Опорные задания и глоссарий — есть \(2\.8\.1\)\.$/.test(State.block('B15').note), 'Б15');
  });

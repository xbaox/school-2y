...
(function () {
  ...
  describe('2.7.0 промпт B7.1: опорные задания без ключей, ключи — в конце', function () {
    ...
    ok(p.indexOf('Основа 1. (L1, 1 mark) ' + tasks[0].q) > 0, 'первое задание с уровнем и баллами');
    ...
    // в ключах — шкала из трёх баллов по дорожке, а не текст задания:
    // текст ученик и так видит в [КОНТЕКСТ]
    ok(keys.indexOf('Письмо (3): 1 — все требуемые термины использованы верно') > 0,
      'критерии письма математической дорожки');
    eq(keys.indexOf(CONTENT.lesson('B7.1').writing), -1, 'текста задания в ключах нет');
  ...
  describe('2.7.0 промпт: без заданий контент говорит об этом прямо', function () {
    school('S0');
    var p = PROMPTS.lesson('B11.1', { today: T });     // задания Б11 приедут пакетом 2.8.1 (Б9 — с 2.8.0)
    ok(p.indexOf('опорные задания придут следующим пакетом контента; ' +
      'вести по фокусу: 4 основных задания уровня L2 школьного; ' +
      'усиление и ⭐⭐ — по правилам') > 0, 'честная строка вместо выдуманных заданий');
  ...
  describe('2.7.0 промпт К.1: конкурсный урок', function () {
    ...
    ok(p.indexOf('Урок: К.1 «Алгебра: уравнения, системы, линейные модели»') > 0,
      'урок подписан как К.1, без дубля номера в заголовке');
  ...
  describe('2.7.0 промпт разминки: долги, запрет слов и одна строка ответа', function () {
    ...
    // образец собран из настоящих id этой разминки, а не из выдуманных
    ok(p.indexOf('«РАЗМИНКА: D-1 ✓ D-2 ✓ D-3 ✓ · слова ') > 0,
      'образец построен из живых id разминки');
    eq(p.indexOf('D-10'), -1, 'выдуманного D-10 в образце нет');

    // в разминку уходит не больше трёх долгов
    eq(State.warmupDebts().length, 3, 'долгов ровно три');
  });

  /* ============ 8b: правки перед деплоем ============ */

  describe('2.7.0 ключи: письмо — критерии из трёх баллов по дорожке', function () {
    school('S0');
    var math = PROMPTS.lesson('B7.1', { today: T });
    var write = PROMPTS.lesson('B8.1', { today: T });

    ok(math.indexOf('Письмо (3): 1 — все требуемые термины использованы верно; ' +
      '1 — полные предложения, одно лицо, present simple, финал называет величину; ' +
      '1 — математически верно') > 0, 'математическая шкала целиком');
    ok(write.indexOf('Письмо (3): 1 — содержание точно по заданию и по тексту; ' +
      '1 — полные предложения своими словами, без копирования; ' +
      '1 — чек-лист языка чист') > 0, 'шкала письма и чтения целиком');

    // а текст задания остаётся в контексте, как и был
    ok(math.indexOf('Письменная работа урока: ' + CONTENT.lesson('B7.1').writing) > 0,
      'текст задания — в [КОНТЕКСТ]');
  });

  describe('2.7.0 глоссарий: у урока без terms печатается запасной', function () {
    school('S0');
    eq(CONTENT.lesson('B3.3').terms, undefined, 'у Б3.3 своих терминов нет');
    var p = PROMPTS.lesson('B3.3', { today: T });
    ok(body(p).indexOf('[ГЛОССАРИЙ]') >= 0, 'блок глоссария всё равно есть');
    ok(p.indexOf('– Point — ') > 0, 'и в нём базовый набор дорожки письма');
    ok(p.indexOf('– Evidence — ') > 0, 'все ключевые понятия абзаца');
    eq(p.indexOf('– domain — '), -1, 'математических терминов туда не попало');

    var m = PROMPTS.lesson('B4.1', { today: T });      // математика без terms
    ok(m.indexOf('– domain — ') > 0, 'у математики свой запасной набор');
    eq(m.indexOf('– Point — '), -1, 'и без понятий абзаца');
  });

  describe('2.7.0 разогрев: слова только своей дорожки', function () {
    school('S0');
    State.applySummary('B8.1', summary({
      words: [{ en: 'main idea', ru: 'главная мысль' }, { en: 'to explain', ru: 'объяснить' }]
    }), { date: '2026-09-12' });
    State.applySummary('B7.1', summary({
      words: [{ en: 'degree', ru: 'степень' }]
    }), { date: '2026-09-13' });

    var math = PROMPTS.lesson('B7.2', { today: T });
    ok(math.indexOf('degree — степень') > 0, 'слово прошлого урока математики на месте');
    eq(math.indexOf('main idea — главная мысль'), -1, 'а слова урока чтения не подмешаны');
    eq(math.indexOf('to explain'), -1, 'и второе тоже');

    var write = PROMPTS.lesson('B8.2', { today: T });
    ok(write.indexOf('main idea — главная мысль') > 0, 'у письма — свои');
    eq(write.indexOf('degree — степень'), -1, 'без математических');
  });

  describe('2.7.0 конкурсный урок: долги и уровень в этапах', function () {
    school('S0');
    var p = PROMPTS.lesson('B53.1', { today: T });
    ok(p.indexOf('Долги ПРИОРИТЕТ проверяются внутри разборов задач по записи решения ' +
      '(М1, М2, М3); «Засчитано» — только за демонстрацию без подсказки.') > 0,
      'строка про долги');
    ok(p.indexOf('Уровень в ИТОГе: L3 — часть B решена полностью; L2 — две задачи из трёх ' +
      'не ниже половины баллов; L1 — иначе.') > 0, 'строка про уровень');
    eq(p.indexOf('студент'), -1, 'слова «студент» в промпте нет');
  });

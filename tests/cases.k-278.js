...
(function () {
  ...
  describe('2.7.8 Б1: свап — К отдельной строкой', function () {
    withToday(MON, function () {
      scene(MATH_OWN);
      var o = swapSheet();
      // 2.8.1: уроки Б16 — математика: у информатики и бизнеса их в свапе нет
      eq(picks(o.body), ['B16.1', 'B8.1', 'B53.1'], 'строки дорожек без К, последняя — К');
      ...
      closeBlock('B53', '2026-09-19');
      o = swapSheet();
      eq(picks(o.body), ['B16.1', 'B8.1'], 'К пройден — выбрать нечего');
    ...
    withToday(MON, function () {
      scene([]);
      var o = swapSheet();
      eq(picks(o.body), ['B7.1', 'B8.1', 'B53.1'], 'свои уроки математики открыты — строка К всё равно последней');
    });
  });

...
(function () {
  ...
  // 2.7.6 (Э6): сегмент выбирает план, очки приходят за достигнутый уровень.
  // До 2.7.6 строка сразу писала очки выбранного уровня. День фиксирован
  // будним: воскресный радар сам по себе дотягивает день до нормы
  describe('статусная строка: уровень объясняет себя', function () { withToday('2026-09-15', function () {
    fresh();
    var t = State.today();
    ...
    State.setLevel('min');
    eq(line(t), 'сегодня: 0 очков · план: карточки + пересказ', 'минималка в плане — очков ещё нет');
    State.day(t).minimalSteps = [true, true];
    State.recount(t);
    eq(line(t), 'сегодня: 1 очко · план: карточки + пересказ', 'минималка сделана — очко');

    State.setLevel('norm');
    eq(line(t), 'сегодня: 1 очко · план: минималка + 1 урок', 'норма в плане, урок не закрыт — очко минималки');
    State.day(t).lessons.push('B7.1');
    State.recount(t);
    eq(line(t), 'сегодня: 2 очка · план: минималка + 1 урок', 'урок закрыт — норма');

    State.setLevel('full');
    eq(line(t), 'сегодня: 2 очка · план: минималка + 2 урока', 'полная в плане при одном уроке — норма');
    State.day(t).lessons.push('B8.1');
    State.recount(t);
    eq(line(t), 'сегодня: 3 очка · план: минималка + 2 урока', 'два урока — полная');
  }); });

  describe('статусная строка: добавки дописываются с итогом', function () { withToday('2026-09-15', function () {
    fresh();
    var t = State.today();
    State.setLevel('full');
    State.toggleAddon('project');
    eq(line(t),
      'сегодня: 0 очков · план: минималка + 2 урока · + Проект (+2) = 2 очка',
      'полная в плане + проект: очки пока только за проект');

    State.s.days[t].lessons = ['B7.1', 'B8.1'];
    State.recount(t);
    State.toggleAddon('radar');
    ok(line(t).indexOf('+ Проект (+2) + Воскресный радар (+1) = 6 очков') > 0,
      'две добавки складываются с очками полной');

    // добавка без выбранного уровня не должна утверждать, что день пустой
    State.setLevel('full');   // снимаем «полную» повторным тапом
    eq([State.day(t).plan, State.day(t).level], ['none', 'none'], 'повторный тап по уровню снимает его');
  ...
  describe('статусная строка: очки берутся из настроек, а не из текста', function () {
    ...
    State.setLevel('norm');
    State.day(t).lessons.push('B1.1');       // норма достигнута — урок закрыт
    State.recount(t);
  ...
  describe('план: минималка — подпись карточек считает колоду дня', function () {
    fresh();
    ...
    // 2.7.8 (Б3): подпись — состав колоды дня, «2 слова + 1 долг» → «2 новых · 1 долг»
    eq(items[0].sub, '2 новых · 1 долг', 'реальные числа со склонением');
  ...
  describe('план: ручные галочки пишутся в день и переживают перерисовку', function () { withToday(MON, function () {
    ...
    eq(State.points(t), 1, 'полная в плане, уроков нет — достигнута минималка: одно очко');
    ...
    eq(State.points(t), 0, 'снятая минималка забирает своё очко');
  }); });
  ...
  describe('план: полный чек-лист радара ставит добавку сам', function () { withToday('2026-08-23', function () {
    ...
    // 2.7.6: радар-день — достигнутый уровень, план «минималка» им набран
    eq(State.points(t), before + 2, 'и дала своё очко по доктрине плюс очко плана минималки');
  ...
  describe('кольцо дня: план = верхний уровень плюс добавки', function () {
    ...
    State.setLevel('full');
    State.day(t).lessons = ['B1.1', 'B2.1'];   // очки плана приходят с уроками (2.7.6)
    State.recount(t);

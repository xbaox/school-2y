...
(function () {
  ...
  describe('долги 2.6.0: id уезжает в промпт', function () {
    ...
    // с 2.7.0 долги печатаются доской по категориям, а не списком «[D-n] текст»
    ok(p.indexOf('М1 ') > 0 && p.indexOf('ОТКРЫТ D-1 ') > 0, 'долг с id на доске категорий');
    ok(p.indexOf('ОТКРЫТ D-2 ') > 0, 'и второй тоже');
    ok(p.indexOf('последний пример: «' + EX[0] + '»') > 0, 'с последним примером ошибки');
    ok(p.indexOf('«Засчитано: [D-…]»') > 0, 'правило 15 контракта объясняет, что с ними делать');
  });
})();

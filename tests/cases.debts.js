...
(function () {
  ...
  /** Три долга дорожки math: D-1, D-2, D-3. */
  function bank() {
    fresh();
  ...
  describe('долги 2.6.0: короткий id выдаётся по порядку', function () {
    ...
    State.applySummary('B1.1', summary({ debts: ['путает however и therefore'] }), { date: '2026-08-21' });
  ...
  describe('долги 2.6.0: сценарий 1 — чистый id', function () {
    ...
    eq(State.matchDebt('[D-1] путает знак наклона', 'math'), d[0], 'id вместе с текстом');
    eq(State.matchDebt('d-2', 'math'), d[1], 'регистр id не важен');

    // id сильнее текста: он якорь, а текст ИИ перефразирует
    eq(State.matchDebt('[D-1] не проверяет ответ подстановкой', 'math'), d[0],
  ...
  describe('долги 2.6.0: сценарий 2 — фолбэк по тексту', function () {
    var d = bank();
    eq(State.matchDebt('путает знак наклона при отрицательном k', 'math'), d[0], 'дословно');
    eq(State.matchDebt('Путает знак наклона при отрицательном K.', 'math'), d[0], 'регистр и точка не мешают');
    eq(State.matchDebt('путает знак наклона при отрицательном k — отработано', 'math'), d[0],
      'формулировка целиком внутри строки');
    eq(State.matchDebt('не проверяет ответы подстановкой', 'math'), d[1],
  ...
  describe('долги 2.6.0: несопоставленные строки видны', function () {
    ...
    var res2 = State.applySummary('B2.3', summary({
      cleared: ['D-2', 'не проверяет ответ подстановкой']
    }), { date: '2026-08-22' });
  ...
  describe('долги 2.6.0: id уезжает в промпт', function () {
    ...
    ok(p.indexOf('[D-1] путает знак наклона при отрицательном k') > 0, 'долг с id в блоке ПАМЯТЬ');
    ok(p.indexOf('[D-2] не проверяет ответ подстановкой') > 0, 'и второй тоже');

...
(function () {
  ...
  // подписи дорожек говорят о ТЕКУЩЕЙ фазе: кейс про Ф0 и стоять должен в Ф0
  describe('2.6.1: дорожка без уроков в текущей фазе', function () { withToday('2026-09-02', function () {
    fresh();
    ...
    // в Ф1 картина другая: своих блоков у бизнеса нет — Б12 уехал на письмо,
    // и дорожке остаётся только общий финальный блок Б16 (track: 'all')
    withToday('2026-09-20', function () {
      eq(State.block('B12').track, 'write', 'Б12 больше не бизнес');
      eq(State.nextLessonInTrack('biz', 'p1'), 'B16.1', 'бизнесу в Ф1 остаются общие финалы');
      eq(Waterfall.hasLessonsNow('write'), true, 'у письма свои уроки есть — Б8');
      eq(Waterfall.hasLessonsNow('math'), true, 'и у математики — Б7');
    });

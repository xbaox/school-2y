...
(function () {
  ...
  describe('промпт B-02: урок без видео', function () {
    ...
    ok(withVideo.indexOf('Видео — только до урока, по карточке «Что смотреть»') > 0,
      'и правило про видео в этапах');
    ok(withVideo.indexOf('урок идёт без него, определения даёт [ГЛОССАРИЙ]') > 0,
      'непросмотренное видео урок не останавливает');
    ok(withVideo.indexOf('О1 проверяется только когда видео просмотрено') > 0,
      'и долг О1 спрашивается только по факту просмотра');
  });
  ...
  describe('промпт B-10: этапы задаёт ступень, а не сжатие 50-минутного урока', function () {
    ...
    ok(s0.indexOf('6. Задание 8/8 — Стретч ⭐⭐ — задача конкурсного уровня') > 0, 'стретч последним');
  ...
  describe('промпт B-06: CEMC ⭐ только в математике', function () {
    ...
    ok(write.indexOf('Стретч ⭐⭐ — задача конкурсного уровня') > 0, 'L3 теперь стретч по желанию');
    State.setMode('summer');
  });

  describe('контракт v3: двадцать два правила дословно', function () {
    fresh();
    ...
    eq(p.indexOf('\n23. '), -1, 'двадцать третьего правила нет');
  ...
  describe('промпт B-07: на Г1–Г3 доля русского названа прямо', function () {
    // с 2.7.3 русский на Г2 не выключается совсем: ≤30% под новыми терминами,
    // а разборы уходят в английский
    fresh('school', 6);         // Г2
    var p = PROMPTS.lesson('B2.1', { today: '2026-09-10' });
    ok(p.indexOf('Доля русского ≤30% — только под новыми терминами, разборы по-английски.') > 0,
      'строка контекста Г2 дословно');
    ...
    eq(p.indexOf('русский выключен'), -1, 'и «русский выключен» тоже: ступени с RU 0 больше нет');

    fresh('school', 5);         // Г1
    var g1 = PROMPTS.lesson('B2.1', { today: '2026-09-10' });
    ok(g1.indexOf('Доля русского ≤10% — только под новыми терминами, разборы по-английски.') > 0,
      'на Г1 та же строка со своей долей');

    fresh('school', 4);         // S4 — доли в контексте нет
    var s4 = PROMPTS.lesson('B2.1', { today: '2026-09-10' });
    eq(s4.indexOf('Доля русского'), -1, 'на S4 доля не печатается');
    State.setMode('summer');
  });
  ...
  describe('промпт B-11: «финиш L3» печатается один раз', function () {
    ...
    ok(p.indexOf('Стретч ⭐⭐ — задача конкурсного уровня') > 0, 'L3 приходит стретчем');

...
(function () {
  ...
  function cardsRow() {
    ...
    return '<section class="block"><h2>Карточки</h2>' +
      '<p class="lead">Колода дня: ' + State.DECK_FRESH + ' мест — слова последнего урока и слова в работе, ' +
      'до трёх открытых долгов, остальное — подошедшие повторы. ' +
      'Не больше ' + State.DECK_CAP + ' карточек — минималка не растёт. ' +
...
/* ============================================================
   ...
   Новые и «в работе» вперёд (2.7.7), тап переворачивает, затем «знал / не знал».
...
window.Cards = (function () {
  ...
  /**
   * Колода дня (2.7.7, Э3): резерв новых и «в работе» → до трёх долгов →
   * подошедшие повторы → добор резерва. Состав и порядок считает
   * State.deckPlan (plan.lead — сколько слов перед долгами); здесь — оформление.
   */
  function deck(todayIso) {
    ...
    var lead = plan.lead == null ? words.length : plan.lead;
    return words.slice(0, lead).concat(debts, words.slice(lead));
  }
   ...
   * 2.7.8 (Б8): остальное ловит отрисовка «Сегодня» (App.autoCards), а снятую
   * руками галочку держит отметка дня day.cardsUntick — её проверяет
   * State.autoCardsStep, общий для обоих путей. Колода открыта и из Журнала,
   * где «Сегодня» не рисуется: отметка и тост здесь — в момент набора.

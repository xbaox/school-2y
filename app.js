...
window.App = (function () {
  ...
  /** «когда»: 1 = понедельник … 7 = воскресенье, как U.weekday. */
  var WD_WHEN = ['в понедельник', 'во вторник', 'в среду', 'в четверг', 'в пятницу', 'в субботу', 'в воскресенье'];

  /**
   * Когда переехавший программный урок снова стоит в плане: ближайший учебный
   * день. 2.7.8 (Б4): учебные дни — U.nextSchoolDay по режиму дня. В «Школе»
   * пн–пт: суббота отдана К, воскресенье — радару, поэтому урок, уступивший
   * пятницу школьному ДЗ, ждёт понедельника, а не субботы. Летом и в «Мосту»
   * суббота — день урока шаблона: из пятницы — «завтра».
   */
  function nextSchoolDayWord(t) {
    var next = U.nextSchoolDay(t, State.lessonMode(t));
    return U.diffDays(t, next) === 1 ? 'завтра' : WD_WHEN[U.weekday(next) - 1];
  }
  ...
  function lessonItem(n, t, d, full) {
    ...
    // 2.8.0 (A4): так же и «Урок 2» полной — кроме субботы: там первым уроком
    // был сам К, и «К.x в субботу» во втором пункте звучало бы про сегодня
    var onlyK = !sel && (n === 1 || U.weekday(t) !== 6) ? contestLeft() : null;
    if (onlyK) {
      return {
        id: 'l' + n, tick: 'lesson', title: head, sub: 'будних уроков нет — ' + lessonCode(onlyK) + ' в субботу',
        done: done,

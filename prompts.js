...
window.PROMPTS = (function () {
  ...
  function stagesBlock(p, youtube, saturday) {
    var yt = youtube || 'подбери сам короткое видео по теме';
    var lines = [
      '[ЭТАПЫ УРОКА]',
      ...
      '3. Вход: видео «' + yt + '» (если не смотрел — отправь смотреть с заданием «3 термина + 1 вопрос»), затем 2 проверочных вопроса + ответ на вопрос студента.',
    ...
    // правило сжатия таймингов (раздел 8.1)
    lines.push('');
    lines.push('Тайминги выше — ориентир для 50-минутного урока. Этот урок ~' + p.lessonMin +
      ...
      '. Просмотр видео в длительность урока не входит (это пауза между этапами 2 и 4), ' +
  ...
  function lessonPrompt(lessonId, opts) {
    ...
    var cemcFlag = p.cemc ? ', плюс 1 задача уровня CEMC ⭐ в математическом уроке' : '';
    var special = p.special ? '\nОсобое на этой ступени: ' + p.special + '.' : '';
    ...
    return [
      ...
      'Доля русского: ' + p.ru + ' (только для новых терминов и грамматики; остальное — английский).' + special,
      'Видео просмотрено: ' + videoDone + ' («' + (lesson.youtube || 'видео не задано') + '»)',
      ...
      '',
      stagesBlock(p, lesson.youtube, saturday),
      '',
  ...
  function minimalPrompt() {
    ...
    var wordText = words.length
      ? words.map(function (w) { return w.en + ' — ' + w.ru; }).join('; ')
      : 'слов пока нет — пройди первый урок, и они появятся';
    var debts = State.openDebts().slice(0, 3);
    var debtText = debts.length ? debts.map(function (d) { return d.text; }).join('; ') : 'нет';

    return [
      'Разминка ~10 минут, без урока и без «ИТОГА».',
      '1) Слова вперемешку, по одному, EN↔RU: ' + wordText,
      '2) Долги — по одному короткому вопросу на каждый: ' + debtText,
      '3) Затем прими мой 60-секундный пересказ сегодняшнего видео/аудио и задай 2 вопроса по нему.',
      'Темп быстрый, объяснения — одной строкой.'
    ].join('\n');
  }
  ...
  return {
    lesson: lessonPrompt, minimal: minimalPrompt, parse: parse,

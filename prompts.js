...
window.PROMPTS = (function () {
  ...
  function contractV3() { return CONTRACT_V3.join('\n'); }
  ...
  function tasksBlock(lesson) {
    ...
    base.forEach(function (t, i) {
      lines.push('Основа ' + (i + 1) + '. (' + (t.level || 'L2') + ', ' + (t.marks || 1) + ' marks) ' + t.q +
        '  /  RU: ' + (t.ru || '—') +
        (t.probe ? '  /  проверяет: ' + t.probe : ''));
    });
    stretch.forEach(function (t) {
      lines.push('Стретч ⭐ (' + (t.level || 'L3') + ', ' + (t.marks || 1) + ' marks) ' + t.q +
        '  /  RU: ' + (t.ru || '—') +
        (t.probe ? '  /  проверяет: ' + t.probe : ''));
    });
    return lines.join('\n');
  ...
  function contestTasksBlock(lesson) {
    ...
    tasks.forEach(function (t, i) {
      lines.push((i + 1) + '. Часть ' + (t.part || 'A') + ' (' + (t.marks || 1) + ' marks) ' +
  ...
  function stageLine(p) {
    ...
    return 'Ступень: ' + p.stepLabel + (p.title && p.title !== p.name ? ' «' + p.title + '»' : '') +
      ' — ' + p.qRange + ' ' + U.plural(parseInt(p.qRange, 10) || 0, 'задание', 'задания', 'заданий') +
  ...
  function minimalPrompt(opts) {
    ...
    lines.push('Последняя строка ответа — одна: ' +
      '«РАЗМИНКА: D-1 ✓ D-10 ✗ · слова 15/15»; ученик вставит только её.');

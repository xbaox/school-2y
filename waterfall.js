...
window.Waterfall = (function () {
  ...
  /**
   * Учебные дни от from до to включительно. 2.7.8 (Б4): счёт — U.schoolDays
   * по режиму дня from (State.lessonMode): в «Школе» пн–пт (суббота — день К,
   * воскресенье — радар-день), летом и в «Мосту» — дни уроков шаблона, пн–сб.
   * mode передаётся явно, когда режим известен заранее.
   */
  function schoolDays(from, to, mode) {
    return U.schoolDays(from, to, mode || State.lessonMode(from));
  }
   ...
   * чем учебных дней (schoolDays) от сегодня до срока включительно: 16.09 у Б7
  ...
  var EXPLAIN = [
    ...
    },
    {
      ...
      cond: 'срок блока сегодня или позади, урок в нём не закрыт — или незакрытых уроков не меньше, чем учебных дней ' +
        'до срока (в «Школе» пн–пт, летом и в «Мосту» пн–сб)',
      act: '→ этот блок'
    },

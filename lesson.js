    ...
    UI.sheet({
      ...
      onMount: function (root, close) {
        ...
        root.querySelector('[data-save]').onclick = function () {
          ...
          // события разбора (ТЗ 2.2): что стало с каждым долгом — по строке.
          // Без этого владелец узнаёт о повторе и сброшенном прогрессе
          // только зайдя в банк долгов
          if ((res.notices || []).length) {
            UI.toast(res.notices.join('\n'), '', 6000);
          }
          // строки «Засчитано», не легшие ни на один долг, показываем отдельно —
